...
function initLineup(name: string, desc: any, _data: any[], lineup?: LineUp) {
  document.querySelector('[data-header="appLink"]').innerHTML =
    'LineUp - ' + name;
  ...
  if (lineup) {
    ...
    lineup = new LineUp(
      document.getElementById('lugui-wrapper'),
      provider,
      lineUpDemoConfig
    );
    lineup
      .addPool(document.getElementById('pool'), {
        hideUsed: false,
        elemWidth: 120,
        elemHeight: 30,
        layout: 'grid',
...

{
  ...
  header.addRightMenu(
    `<span title="Download CSV"><i class="fa fa-download"></i><sub class="fa fa-file-excel-o"></sub></span>`,
    () => {
      if (lineup) {
  ...
  header.addRightMenu(
    `<span title="Download JSON"><i class="fa fa-download"></i><sub class="fa fa-file-code-o"></sub></span>`,
    () => {
      if (lineup) {
  ...
  header.addRightMenu(
    `<span title="Upload to Github Gist"><i class="fa fa-cloud-upload"></i><sub class="fa fa-github"></sub></span>`,
    () => {
      if (lineup) {
  ...
  header.rightMenu.insertBefore(
    document.getElementById('datasetSelector'),
    header.rightMenu.firstChild
  );
  header.addRightMenu(
    `<span title="Upload CSV/JSON"><i class="fa fa-upload"></i><sub><i class="fa fa-file-excel-o"></i><i class="fa fa-file-code-o"></i></sub></span>`,
    () => {
      importFile()
        .then(({ name, desc, data }) => {
